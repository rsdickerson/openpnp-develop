...
public class NamedCSVImporter implements BoardImporter {
	 ...
	 * System.out.println("Usage: java CSVParseDemo <csv_file>"); return; }
	 * 
	 * CSVParser parser = new CSVParser(new FileReader(args[0]),
	 * CSVStrategy.DEFAULT); String[] values = parser.getLine(); while (values
	 * != null) { printValues(parser.getLineNumber(), values); values =
	 * parser.getLine(); } }
	 * 
	...
	private static final String Refs[] = { "Designator", "designator", "Part", "part", "Component", "component",
			"RefDes", "Ref" };
	...
	static private int units_mils_x = 0, units_mils_y = 0, units_mils_height = 0; // set
																					// if
																					// units
																					// are
																					// in
		...
		for (int i = 0; i < str.length; i++) {
			for (int j = 0; j < val.length; j++) {
				if (str[i].equals(val[j])) {
					...
					// check for mil units
					// TODO This should be done better, but at moment I don't
					// know a better way...
					// -trampas
	...
	private static boolean checkCSV(String str[]) {
		...
		// note that layer (TB) and Height (HT) are optional and thus checked
		// against -2
		if ((Ref = checkCSV(Refs, str)) != -1 && (Val = checkCSV(Vals, str)) != -1
				&& (Pack = checkCSV(Packs, str)) != -1 && (X = checkCSV(Xs, str)) != -1 && (Y = checkCSV(Ys, str)) != -1
	...
	private static boolean checkLine(String str) throws Exception {
		...
		if (str.indexOf("Designator") == -1 && str.indexOf("designator") == -1 && str.indexOf("Part") == -1
				&& str.indexOf("part") == -1 && str.indexOf("Component") == -1 && str.indexOf("component") == -1
	...
	private static List<Placement> parseFile(File file, boolean createMissingParts, boolean updateHeights)
		...
		for (String as[]; (as = csvParser.getLine()) != null;) {
			if (as.length <= Len) {
				...
				double heightZ = 0.0; // set default height to zero in case not
										// included in CSV
				if (HT != -1) {
					heightZ = Double.parseDouble(
							as[HT].replace(",", ".").replace(" ", "").replace("mm", "").replace("mil", ""));
				...
				placement.setLocation(
						new Location(LengthUnit.Millimeters, placementX, placementY, 0, placementRotation));
				...
				if (cfg != null && createMissingParts) {
					...
					// if part exists and height exist and user wants height
					// updated do it.
	...
	class Dlg extends JDialog {
		...
		public Dlg(Frame parent) {
			...
			// TODO the placement of the checkbox is not pretty, big space
			// between it and one above,
