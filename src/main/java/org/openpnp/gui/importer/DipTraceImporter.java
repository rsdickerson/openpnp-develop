...
public class DipTraceImporter implements BoardImporter {
	...
	private static List<Placement> parseFile(File file, boolean createMissingParts) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			double placementX = Double.parseDouble(tokens[2]); // X (mm) in
																// Diptrace
																// export
			double placementY = Double.parseDouble(tokens[3]); // Y (mm) in
																// Diptrace
																// export
			double placementRotation = Double.parseDouble(tokens[5]); // Rotate
																		// in
																		// Diptrace
																		// export
		...
		private class SwingAction_2 extends AbstractAction {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					MessageBoxes.errorBox(Dlg.this, "Import Error",
							"The expected file format is the default file export in DipTrace "
