...
public class MachineControlsPanel extends JPanel {
	...
	public MachineControlsPanel(Configuration configuration) {
		setBorder(new TitledBorder(null, "Machine Controls", TitledBorder.LEADING, TitledBorder.TOP, null, null));
	...
	/**
	 * Currently returns the selected Nozzle. Intended to eventually return
	 * either the selected Nozzle or PasteDispenser.
	 * 
	 * @return
	 */
	...
	public Action startStopMachineAction = new AbstractAction("Stop", Icons.powerOn) {
		...
		public void actionPerformed(ActionEvent arg0) {
			setEnabled(false);
			// Note: We specifically bypass the machine submit so that this runs
			// immediately.
			// That's not really thread safe tho, so it's better than nothing,
			// but not much.
	...
	private void updateStartStopButton(boolean enabled) {
		...
		startStopMachineAction.putValue(Action.SMALL_ICON, enabled ? Icons.powerOffNoHome : Icons.powerOn);
	}
