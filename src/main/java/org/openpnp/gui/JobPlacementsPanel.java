...
public class JobPlacementsPanel extends JPanel {
	...
	public JobPlacementsPanel(JobPanel jobPanel) {
		...
		singleSelectionActionGroup = new ActionGroup(removeAction, editPlacementFeederAction, setTypeAction);
		singleSelectionActionGroup.setEnabled(false);

		multiSelectionActionGroup = new ActionGroup(removeAction, setTypeAction);
		multiSelectionActionGroup.setEnabled(false);

		captureAndPositionActionGroup = new ActionGroup(captureCameraPlacementLocation, captureToolPlacementLocation,
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (getSelections().size() > 1) {
					...
					captureAndPositionActionGroup
							.setEnabled(getSelection() != null && getSelection().getSide() == boardLocation.getSide());
					Configuration.get().getBus()
							.post(new PlacementSelectedEvent(getSelection(), boardLocation, JobPlacementsPanel.this));
				}
			}
		});
	...
	public final Action newAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			String id = JOptionPane.showInputDialog(getTopLevelAncestor(), "Please enter an ID for the new placement.");
			if (id == null) {
				return;
			}
	...
	public final Action captureCameraPlacementLocation = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent arg0) {
			UiUtils.messageBoxOnException(() -> {
				...
				Location placementLocation = Utils2D.calculateBoardPlacementLocationInverse(boardLocation,
						camera.getLocation());
