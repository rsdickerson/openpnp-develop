...
public class JogControlsPanel extends JPanel {
	...
	private void park(boolean xy, boolean z, boolean c) {
		UiUtils.submitUiMachineTask(() -> {
			...
			location = location.derive(xy ? parkLocation.getX() : null, xy ? parkLocation.getY() : null,
					z ? parkLocation.getZ() : null, c ? parkLocation.getRotation() : null);
	...
	private void createUi() {
		...
		// We set this Icon explicitly as a WindowBuilder helper. WindowBuilder
		// can't find the
		// homeAction referenced above so the icon doesn't render in the viewer.
		// We set it here
	...
	public Action discardAction = new AbstractAction("Discard") {
		...
		public void actionPerformed(ActionEvent arg0) {
			UiUtils.submitUiMachineTask(() -> {
				...
				MovableUtils.moveToLocationAtSafeZ(nozzle, Configuration.get().getMachine().getDiscardLocation());
				// discard the part
				nozzle.place();
				nozzle.moveToSafeZ();
			});
