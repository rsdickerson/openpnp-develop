...
public class PartCellValue implements Comparable<PartCellValue> {
	...
	public int compareRefName(String s1, String s2) {
		...
		if (numericPos(s1) != -1 && numericPos(s2) != -1) {
			String s1First = s1.substring(0, numericPos(s1)); // L123 this would
																// contain L
			String s2First = s2.substring(0, numericPos(s2)); // L256 this would
																// contain L

			if (s1First.compareTo(s2First) == 0) {
				String s1Second = s1.substring(numericPos(s1), s1.length()); // L123
																				// this
																				// would
																				// contain
																				// 123
				String s2Second = s2.substring(numericPos(s2), s2.length()); // L256
																				// this
																				// would
																				// contain
																				// 256
