...
public class LogPanel extends JPanel {
	...
	private JCheckBox createSystemOutputCheckbox() {
		...
		systemOutCheckbox.addActionListener(e -> {
			...
			if (systemOutEnabled) {
				...
				systemOutFilter
						.setFilter(logEntry -> !Objects.equals(logEntry.getClassName(), SystemLogger.class.getName()));
			}
			logEntries.filter();
		});
	...
	private void loadLoggingPreferences() {
		...
		// This weird check is here because I mistakenly reused the same config
		// key when
		// switching from slf to tinylog. This meant that some users had an int
		// based
		// value in the key rather than the string. This caused initialization
		// failures.
		Level level = null;
		try {
		...
		Configurator.currentConfig().addWriter(logEntries).activate();
	}
	...
	private JPanel createSearchFieldPanel() {
		...
		searchTextField.getDocument().addDocumentListener(new DocumentListener() {
			...
			private void updateSearchBarFilter() {
				...
				searchBarFilter.setFilter(
						(logEntry -> logEntry.getRenderedLogEntry().toLowerCase().contains(searchText.toLowerCase())));
				logEntries.filter();
			}
	...
	private JPanel createGlobalLogLevelPanel() {
		...
		JComboBox<Level> logLevelComboBox = createLogLevelFilterComboBox(
				Level.valueOf(prefs.get(PREF_LOG_LEVEL, PREF_LOG_LEVEL_DEF)));
		logLevelComboBox.addActionListener(e -> {
			...
			Configurator.currentConfig().level(logLevel).activate();
		});
