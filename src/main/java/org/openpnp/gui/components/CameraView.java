...
public class CameraView extends JComponent implements CameraListener {
	 ...
	 * the specified time.
	 * 
	 * In addition to showing the given image, if the text parameters is not
	 * null the text will be shown during the timeout using setText().
	 * 
	 * @param image
	 * @param text
	 * @param millseconds
	...
	public void frameReceived(BufferedImage img) {
		...
		lastFrame = img;
		if (oldFrame == null || (oldFrame.getWidth() != img.getWidth() || oldFrame.getHeight() != img.getHeight()
	...
	protected synchronized void paintComponent(Graphics g) {
		...
		if (image != null) {
			...
			for (Reticle reticle : reticles.values()) {
				reticle.draw(g2d, camera.getUnitsPerPixel().getUnits(), scaledUnitsPerPixelX, scaledUnitsPerPixelY,
						ins.left + (width / 2), ins.top + (height / 2), scaledWidth, scaledHeight, c);
			}
	...
	private void paintSelection(Graphics2D g2d) {
		...
		g2d.setColor(Color.white);
		g2d.setStroke(new BasicStroke(1f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, selectionDashProfile,
				selectionDashPhase));
		g2d.drawRect(rx, ry, rw, rh);
	...
	private static boolean isWithin(int pointX, int pointY, int boundsX, int boundsY, int boundsWidth,
			int boundsHeight) {
	...
	private void drawImageInfo(Graphics2D g2d, int topLeftX, int topLeftY, BufferedImage image) {
		if (image == null) {
			return;
		}
		...
		g2d.setColor(new Color(0, 0, 0, 0.75f));
		g2d.fillRoundRect(topLeftX, topLeftY, width + insets.left + insets.right, height + insets.top + insets.bottom,
				cornerRadius, cornerRadius);
		g2d.setColor(Color.white);
		g2d.drawRoundRect(topLeftX, topLeftY, width + insets.left + insets.right, height + insets.top + insets.bottom,
				cornerRadius, cornerRadius);
		int yPen = topLeftY + insets.top;
		for (TextLayout textLayout : textLayouts) {
			yPen += textLayout.getBounds().getHeight();
	...
	private void fireActionEvent(MouseEvent e) {
		...
		CameraViewActionEvent action = new CameraViewActionEvent(CameraView.this, e.getX(), e.getY(),
	...
	public CameraViewSelectionTextDelegate pixelsAndUnitsTextSelectionDelegate = new CameraViewSelectionTextDelegate() {
		...
		public String getSelectionText(CameraView cameraView) {
			...
			String text = String.format(Locale.US, "%dpx, %dpx\n%2.3f%s, %2.3f%s", (int) selection.getWidth(),
					(int) selection.getHeight(), widthInUnits, camera.getUnitsPerPixel().getUnits().getShortName(),
					heightInUnits, camera.getUnitsPerPixel().getUnits().getShortName());
			return text;
		}
	};
}
