...
public class SubmitDiagnosticsDialog extends JDialog {
	...
	public SubmitDiagnosticsDialog() {
		...
		}
		{
			...
			txtpnWarningIfYou.setText(
					"Warning: If you include a screenshot or Vision Debug Images these images may include output from your machine's cameras. If these images contain content you don't want to share you should uncheck these options. You can review the images from the generated link before sharing it.");
		...
		// We have to grab a screenshot before the dialog is shown or the
		// screenshot
		// will include the dialog. So we grab it and if we don't need it it
		// just gets
		// ignored.
		try {
	...
	public Action sendAction = new AbstractAction("Send") {
		...
		public void actionPerformed(ActionEvent e) {
			/**
			 * Hide the dialog Grab screenshot Show the dialog Start send
			 * Disable Send Cancel cancels... Update progress bar
			 */
			...
			thread = new Thread(() -> {
				try {
					...
					if (!images.isEmpty()) {
						...
						album = imgur.createAlbum("OpenPnP Diagnostics Images", albumImages.toArray(new Image[] {}));
						progressBar.setValue(progressBar.getValue() + 1);
					}

					Gist gist = new Gist();
					gist.setDescription(String.format("OpenPnP Diagnostics: %s; Images: %s", descriptionTa.getText(),
	...
	static String getSystemInfo() throws Exception {
		...
		String[] keys = new String[] { "os.name", "os.arch", "java.runtime.name", "java.vm.vendor", "java.vm.name",
				"user.country", "java.runtime.version", "os.version", "java.vm.info", "java.version", };
		for (String key : keys) {
