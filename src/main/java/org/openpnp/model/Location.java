...
public class Location {
	...
	public Location convertToUnits(LengthUnit units) {
		Location location = new Location(units, new Length(x, this.units).convertToUnits(units).getValue(),
	...
	/**
	 * Returns a new Location with the given Location's X, Y, and Z components
	 * added to this Location's X, Y, and Z components. Rotation is included.
	 * 
	 * @param l
	 * @return
	 */
	...
	public Location invert(boolean x, boolean y, boolean z, boolean rotation) {
		return new Location(getUnits(), getX() * (x ? -1 : 1), getY() * (y ? -1 : 1), getZ() * (z ? -1 : 1),
	...
	public Location derive(Double x, Double y, Double z, Double rotation) {
		return new Location(units, x == null ? this.x : x, y == null ? this.y : y, z == null ? this.z : z,
