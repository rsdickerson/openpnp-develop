...
public class EagleLoader {
	...
	public Eagle eagle; // TODO remove eagle as this is not strictly required as
						// we peel out the
	...
	public EagleLoader(InputStream in) throws Exception {
		...
		eagle = (Eagle) unmarshaller.unmarshal(source); // TODO change later to
														// Eagle eagle =
														// (Eagle)
		...
		try {
			...
			for (Package pkg : packages.getPackage()) {
				...
				for (Object o : pkg.getPolygonOrWireOrTextOrDimensionOrCircleOrRectangleOrFrameOrHoleOrPadOrSmd()) {
					if (o instanceof Smd) {
						Smd smd = (Smd) o;
						System.out.println(String.format("<pad x=\"%s\" y=\"%s\" width=\"%s\" height=\"%s\"/>",
