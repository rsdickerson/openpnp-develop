...
public class Panel extends AbstractModelObject implements Identifiable {
	...
	// This constructor is used for creating a pcb Panel with two fiducials. In
	// this first release,
	// we only contemplate UI
	// that supports two fids on a panel

	public Panel(String id, int cols, int rows, Length xGap, Length yGap, String partId, boolean checkFids,
			Placement fid0, Placement fid1) {
		this(id);
		this.columns = cols;
		this.rows = rows;
		this.xGap = xGap;
	...
	public void setLocation(Job job) {
		...
		for (int j = 0; j < getRows(); j++) {
			for (int i = 0; i < getColumns(); i++) {
				...
				// OFfset the sub PCB
				newPCB.setLocation(newPCB.getLocation().add(new Location(Configuration.get().getSystemUnits(),
						(pcbWidthX + getXGap().getValue()) * i, (pcbHeightY + getYGap().getValue()) * j, 0, 0)));
