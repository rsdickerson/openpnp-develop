...
public class Job extends AbstractModelObject implements PropertyChangeListener {
	...
	// In the first release of the Auto Panelize software, there is assumed to
	// be a
	// single panel in use, even though the underlying plumbing supports a list
	// of
	...
	public boolean isUsingPanel() {
		...
		if ((panels.size() >= 1) && ((panels.get(0).getRows() > 1) || (panels.get(0).getColumns() > 1))) {
			return true;
		}
