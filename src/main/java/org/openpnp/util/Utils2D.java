...
public class Utils2D {
	public static Point rotateTranslateScalePoint(Point point, double c, double x, double y, double scaleX,
			double scaleY) {
	...
	public static Location calculateBoardPlacementLocation(BoardLocation bl, Location placementLocation) {
		return calculateBoardPlacementLocation(bl.getLocation(), bl.getSide(), bl.getBoard().getDimensions().getX(),
				placementLocation);
	}

	public static Location calculateBoardPlacementLocation(Location boardLocation, Side side, double offset,
			Location placementLocation) {
		// The Z value of the placementLocation is always ignored, so zero it
		// out to make sure.
	...
	public static Location calculateBoardPlacementLocationInverse(BoardLocation boardLocation,
			Location placementLocation) {
		return calculateBoardPlacementLocationInverse(boardLocation.getLocation(), boardLocation.getSide(),
				boardLocation.getBoard().getDimensions().getX(), placementLocation);
	}

	public static Location calculateBoardPlacementLocationInverse(Location boardLocation, Side side, double offset,
			Location placementLocation) {
	...
	/**
	 * Given an existing BoardLocation, two Placements and the observed location
	 * of those two Placements, calculate the actual Location of the
	 * BoardLocation. Note that the BoardLocation is only used to determine
	 * which side of the board the Placements are on - it's existing Location is
	 * not considered. The returned Location is the absolute Location of the
	 * board, including it's angle, with the Z value set to the Z value in the
	 * input BoardLocation.
	 * 
	 * @param boardLocation
	 * @param placementA
	...
	public static Location calculateBoardLocation(BoardLocation boardLocation, Placement placementA,
		...
		// Location. This will be used to calculate our ideal placement
		// locations.
		...
		// Calculate the ideal placement locations. This is where we would
		// expect the
		...
		double actualAngle = Math
				.toDegrees(Math.atan2(actualB.getY() - actualA.getY(), actualB.getX() - actualA.getX()));

		// The difference in angles is the angle of the board
		double angle = actualAngle - idealAngle;

		// Now we rotate the first placement by the angle, which gives us the
		// location
		// that the placement would be had the board been rotated by that angle.
		Location idealARotated = idealA.rotateXy(angle);

		// And now we subtract that rotated location from the observed location
		// to get
	...
	/**
	 * Calculate the Location along the line formed by a and b with distance
	 * from a.
	 * 
	 * @param a
	 * @param b
	...
	static public double getAngleFromPoint(Location firstPoint, Location secondPoint) {
		...
		if ((secondPoint.getX() > firstPoint.getX())) {
			angle = (Math.atan2((secondPoint.getX() - firstPoint.getX()), (firstPoint.getY() - secondPoint.getY()))
					* 180 / Math.PI);
		}
		// above 180 degrees to 360/0
		else if ((secondPoint.getX() <= firstPoint.getX())) {
			angle = 360
					- (Math.atan2((firstPoint.getX() - secondPoint.getX()), (firstPoint.getY() - secondPoint.getY()))
							* 180 / Math.PI);
		}
		return angle;
	}
