...
public class OpenCvUtils {
	...
	public static BufferedImage toBufferedImage(Mat m) {
		...
		if (m.type() == CvType.CV_8UC1) {
			...
			// Copy the results into the original Mat and release our temp copy
			// so that when
		...
		if (type == null) {
			throw new Error(String.format("Unsupported Mat: type %d, channels %d, depth %d", m.type(), m.channels(),
					m.depth()));
		}
	 ...
	 * @throws Exception
	 */
	public static List<Location> houghCircles(Camera camera, Length minDiameter, Length maxDiameter, Length minDistance)
			throws Exception {
	...
	public static Mat houghCircles(Mat mat, double minDiameter, double maxDiameter, double minDistance) {
		...
		Mat circles = new Mat();
		Imgproc.HoughCircles(mat, circles, Imgproc.CV_HOUGH_GRADIENT, 1, minDistance, 80, 10, (int) (minDiameter / 2),
				(int) (maxDiameter / 2));
	...
	public synchronized static void saveDebugImage(Class implementationClass, String function, String identifier,
			BufferedImage img) {
		if (img == null) {
			return;
		}
		if (LogUtils.isDebugEnabled()) {
			try {
				...
				file = new File(file, String.format("%s_%s_%s_%s.png", implementationClass.getSimpleName(), function,
						df.format(new Date()), identifier));
