...
/**
 * A CvPipeline performs computer vision operations on a working image by
 * processing in series a list of CvStage instances. Each CvStage instance can
 * modify the working image and return a new image along with data extracted
 * from the image. After processing the image callers can get access to the
 * images and models from each stage.
 * 
 * CvPipeline is serializable using toXmlString and fromXmlString. This makes it
 * easy to export pipelines and exchange them with others.
 * 
...
public class CvPipeline {
	...
	/**
	 * Add the given CvStage to the end of the pipeline using the given name. If
	 * name is null a unique one will be generated and set on the stage.
	 * 
	 * @param name
	 * @param stage
	 */
	...
	/**
	 * Add the given CvStage to the end of the pipeline. If the stage does not
	 * have a name a unique one will be generated and set on the stage.
	 * 
	 * @param stage
	 */
	...
	/**
	 * Get the Result returned by the CvStage with the given name. May return
	 * null if the stage did not return a result.
	 * 
	 * @param name
	 * @return
	 */
	...
	/**
	 * Get the Result returned by give CvStage. May return null if the stage did
	 * not return a result.
	 * 
	 * @param stage
	 * @return
	...
	/**
	 * Get the current working image. Primarily intended to be called from
	 * CvStage implementations.
	 * 
	 * @return
	 */
	...
	public void process() {
		...
		for (CvStage stage : stages) {
			...
			// If the result image is null and there is a working image, replace
			// the result image
	...
	/**
	 * Release any temporary resources associated with the processing of the
	 * pipeline. Should be called when the pipeline is no longer needed. This is
	 * primarily to release retained native resources from OpenCV.
	 */
	public void release() {
		if (workingImage != null) {
			workingImage.release();
	...
	/**
	 * Convert the pipeline to an XML string that can be read back in with
	 * #fromXmlString.
	 * 
	 * @return
	 * @throws Exception
	...
	/**
	 * Parse the pipeline in the given String and replace the current pipeline
	 * with the results.
	 * 
	 * @param s
	 * @throws Exception
