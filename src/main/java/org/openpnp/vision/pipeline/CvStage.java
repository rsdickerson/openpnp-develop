...
/**
 * Base class for a stage in a CvPipeline. A CvStage has a unique name within a
 * pipeline and is able to perform computer vision operations reuslting in
 * either a modified working image or a new image and optional model data
...
public abstract class CvStage {
	 ...
	 * CvPipeline#getWorkingImage(), perform some type of operation on the image
	 * and will return a Result containing a modified image and model data about
	 * features found in the image.
	 *
	 * If the stage only modifies the working image, it is sufficient to just
	 * return null, and this will typically be the most common case.
	 * 
	 * @param pipeline
	 * @return Null or a Result object containing an optional image and optional
	 *         model. If the return value is null the pipeline will store a copy
	 *         of the working image as the result for this stage. Otherwise it
	 *         will set the working image to the result image and store the
	 *         result image.
	 * @throws Exception
	 */
	...
	public static class Result {
		...
		public static class TemplateMatch {
			...
			@Override
			public String toString() {
				return "TemplateMatch [x=" + x + ", y=" + y + ", width=" + width + ", height=" + height + ", score="
						+ score + "]";
			}
		}
	}
