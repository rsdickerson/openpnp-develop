...
/**
 * Draws RotatedRects from a stage's model. Input can be either a single
 * RotatedRect or a List<RotatedRect>
 */
public class DrawRotatedRects extends CvStage {
	...
	public Result process(CvPipeline pipeline) throws Exception {
		...
		if (result.model instanceof RotatedRect) {
			FluentCv.drawRotatedRect(mat, ((RotatedRect) result.model),
					color == null ? FluentCv.indexedColor(0) : color, thickness);
