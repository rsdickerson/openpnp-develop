...
public class FilterRects extends CvStage {
	...
	public Result process(CvPipeline pipeline) throws Exception {
		...
		for (RotatedRect rect : rects) {
			...
			if (sizeType == 2) {
				// Any size:
				// Select rects based only on the aspect ratio of each input
				// rectangle
				lmin = rl;
				lmax = rl;
				wmax = rl * armax;
			...
			// because specified width and length limits take precedence over
			// aspect ratio limits
			...
			if (enableLogging) {
				Logger.info("{} rect: xy={}, {} area={} angle= {}°", pass, String.format("%4d", (int) rect.center.x),
						String.format("%-4d", (int) rect.center.y), String.format("%6d", (int) (rw * rl)),
						String.format("%4d", (int) rect.angle));
				Logger.info("    {} widths : {} <= {} <= {}, mean= {}", (wok ? " " : "!"), String.format("%.3f", wmin),
						String.format("%.3f", rw), String.format("%.3f", wmax),
						String.format("%.3f", (wmin + wmax) / 2.0));
				Logger.info("    {} lengths: {} <= {} <= {}, mean= {}", (lok ? " " : "!"), String.format("%.3f", lmin),
						String.format("%.3f", rl), String.format("%.3f", lmax),
						String.format("%.3f", (lmin + lmax) / 2.0));
				Logger.info("    {} aspectR: {} <= {} <= {}, mean= {}", (sizeType == 3 || arok ? " " : "!"),
						String.format("%.3f", armin), String.format("%.3f", rar), String.format("%.3f", armax),
