 ...
 * from FireSight: https://github.com/firepick1/FireSight. Scans the working
 * image for matches of a template image and returns a list of matches.
 */
public class MatchTemplate extends CvStage {
	...
	/**
	 * If maxVal is below this value, then no matches will be reported. Default
	 * is 0.7.
	 */
	@Attribute
	 ...
	 * best match of candidates. For CV_TM_CCOEFF, CV_TM_CCOEFF_NORMED,
	 * CV_TM_CCORR, and CV_TM_CCORR_NORMED methods, this is a minimum threshold
	 * for positive recognition; for all other methods, it is a maximum
	 * threshold. Default is 0.85.
	 */
	@Attribute
						...
						if (rangeMin <= curVal && curVal <= rangeMax) { // ROW
							...
							if (0 < r && (mat.get(r - 1, c - 1)[0] >= curVal || mat.get(r - 1, c)[0] >= curVal)) {
								...
								// - - -
									...
									&& (mat.get(r + 1, c - 1)[0] > curVal || mat.get(r + 1, c)[0] > curVal)) {
								// cout << "reject:r+1 " << r << "," << c-1 <<
								// endl;
								// - - -
								// - - -
								// - x x
									...
									&& (0 < r && mat.get(r - 1, c - 2)[0] >= curVal || mat.get(r, c - 2)[0] > curVal
			...
			if (state == MinMaxState.BEFORE_INFLECTION) {
				if (rangeMin <= curVal && curVal <= rangeMax) { // ROW MAXIMA
					if (0 < r && (mat.get(r - 1, cEnd - 1)[0] >= curVal || mat.get(r - 1, cEnd)[0] >= curVal)) {
						...
						// - - -
						// - x x
					} else if (1 < r && mat.get(r - 1, cEnd - 2)[0] >= curVal || mat.get(r, cEnd - 2)[0] > curVal
