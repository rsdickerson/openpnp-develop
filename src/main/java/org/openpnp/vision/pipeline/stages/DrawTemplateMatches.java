...
/**
 * Draws TemplatesMatches contained in a List<TemplateMatch> by referencing a
 * previous stage's model data.
 */
public class DrawTemplateMatches extends CvStage {
	...
	public Result process(CvPipeline pipeline) throws Exception {
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			Core.rectangle(mat, new org.opencv.core.Point(x, y), new org.opencv.core.Point(x + width, y + height),
					color);
			Core.putText(mat, "" + score, new org.opencv.core.Point(x + width, y + height), Core.FONT_HERSHEY_PLAIN,
					1.0, color);
		}
