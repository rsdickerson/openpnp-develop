...
public class MatView extends JComponent {
	...
	protected void paintComponent(Graphics g) {
		...
		/**
		 * We want to fit both axes in the given destWidth and destHeight while
		 * maintaining the aspect ratio. If the frame is smaller in either or
		 * both axes than the original will need to be scaled to fill the space
		 * as completely as possible while still maintaining the aspect ratio.
		 * 1. Determine the source size of the image: sourceWidth, sourceHeight.
		 * 2. Determine the max size each axis can be: destWidth, destHeight. 3.
		 * Calculate how much each axis needs to be scaled to fit. 4. Use the
		 * larger of the two and scale the opposite axis by the aspect ratio +
		 * the scaling ratio.
		 */
