...
public class ResultsPanel extends JPanel {
		...
		matView.addMouseMotionListener(new MouseAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				matStatusLabel.setText(String.format("RGB: %03d, %03d, %03d HSB: %03d, %03d, %03d XY: %d, %d",
						color.getRed(), color.getGreen(), color.getBlue(), (int) (255.0 * hsb[0]),
						(int) (255.0 * hsb[1]), (int) (255.0 * hsb[2]), p.x, p.y));
			}
		});
	...
	public void refresh() {
		...
		// Otherwise if nothing is selected or if the previously selected stage
		// is no longer in the
	...
	private void updateAllEverything() {
		...
		resultStageNameLabel.setText(result == null || selectedStage == null ? ""
				: (selectedStage.getName() + " (" + (result.processingTimeNs / 1000000.0) + " ms)"));
