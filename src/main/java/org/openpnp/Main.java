...
public class Main {
	...
	private static void configureLogging(File configurationDirectory) {
		...
		Configurator.currentConfig().writer(new RollingFileWriter(logFile.getAbsolutePath(), 100))
				.addWriter(new ConsoleWriter(System.out, System.err)).activate();
		Configurator.currentConfig().formatPattern("{date:yyyy-MM-dd HH:mm:ss} {class_name} {level}: {message}")
				.activate();

		// Redirect the stdout and stderr to the LogPanel
		// TODO: Temporarily commented out because of stack overflows on
		// shutdown
		// on Windows. See:
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					Logger.debug(String.format("Bienvenue, Willkommen, Hello, Namaskar, Welkom to OpenPnP version %s.",
							Main.getVersion()));
