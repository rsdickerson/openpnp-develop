...
public class OpenBuildsDriver extends AbstractSerialPortDriver implements Runnable {
	...
	public Location getLocation(ReferenceHeadMountable hm) {
		if (hm instanceof ReferenceNozzle) {
			...
			return new Location(LengthUnit.Millimeters, x, y, z, Utils2D.normalizeAngle(nozzleIndex == 0 ? c : c2))
					.add(hm.getHeadOffsets());
		} else {
			return new Location(LengthUnit.Millimeters, x, y, zA, Utils2D.normalizeAngle(c)).add(hm.getHeadOffsets());
		}
	}
	...
	public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception {
		...
		if (!Double.isNaN(c) && c != oldC) {
			...
			// Get the delta between the current position and the new position
			// in normalized
			// degrees.
			double delta = c - Utils2D.normalizeAngle(oldC);

			// If the delta is greater than 180 we'll go the opposite direction
			// instead to
			...
			// We perform E moves solo because Smoothie doesn't like to make
			// large E moves
			// with small X/Y moves, so we can't trust it to end up where we
			// want it if we
