...
public class GcodeDriverConsole extends AbstractConfigurationWizard {
	...
	public GcodeDriverConsole(GcodeDriver driver) {
		...
		gcodeConsole.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Gcode console",
	...
	private void sendGcodeConCmd() {
		...
		// are we repeating the same command? Store it in history only if
