...
public class GcodeDriver extends AbstractSerialPortDriver implements Runnable {
	public enum CommandType {
		COMMAND_CONFIRM_REGEX, POSITION_REPORT_REGEX, COMMAND_ERROR_REGEX, CONNECT_COMMAND, ENABLE_COMMAND, DISABLE_COMMAND, POST_VISION_HOME_COMMAND, HOME_COMMAND(
				"Id", "Name"), PUMP_ON_COMMAND, PUMP_OFF_COMMAND, MOVE_TO_COMMAND(true, "Id", "Name", "FeedRate", "X",
						"Y", "Z", "Rotation"), MOVE_TO_COMPLETE_REGEX(true), PICK_COMMAND(true, "Id", "Name",
								"VacuumLevelPartOn",
								"VacuumLevelPartOff"), PLACE_COMMAND(true, "Id", "Name"), ACTUATE_BOOLEAN_COMMAND(true,
										"Id", "Name", "Index", "BooleanValue", "True", "False"), ACTUATE_DOUBLE_COMMAND(
												true, "Id", "Name", "Index", "DoubleValue", "IntegerValue"),
		// TODO: Remove the vacuum stuff after 2017-05-01, see
		// https://github.com/openpnp/openpnp/issues/447
		VACUUM_REQUEST_COMMAND(true, "VacuumLevelPartOn", "VacuumLevelPartOff"), VACUUM_REPORT_REGEX(
				true), ACTUATOR_READ_COMMAND(true, "Id", "Name", "Index"), ACTUATOR_READ_REGEX(true);
	...
	public void home(ReferenceHead head) throws Exception {
		// Home is sent with an infinite timeout since it's tough to tell how
		// long it will
		// take.
		 ...
		 * correctly) directly above the homing pin in the machine bed, use the
		 * head camera scan for this and make sure this is exactly central -
		 * otherwise we move the camera until it is, and then reset all the axis
		 * back to 0,0,0,0 as this is calibrated home.
		 */
		...
		if (homePart != null) {
			...
			// homeOffset contains the offset, but we are not really concerned
			// with that,
	...
	public Location getLocation(ReferenceHeadMountable hm) {
		...
		// additional info might be on subdrivers (note that subdrivers can only
		// be one level deep)
		for (ReferenceDriver driver : subDrivers) {
	...
	public void moveTo(ReferenceHeadMountable hm, Location location, double speed) throws Exception {
		...
		// Handle NaNs, which means don't move this axis for this move. We set
		// the appropriate
		...
		if (xAxis != null || yAxis != null || zAxis != null || rotationAxis != null) {
			...
			// For each included axis, if the axis has a transform, transform
			...
			// If Y is moving and there is a non squareness factor we also need
			// to move X, even if
			...
			if (includeX) {
				...
				command = substituteVariable(command, "BacklashOffsetX", x + backlashOffsetX + nonSquarenessFactor * y); // Backlash
																															// Compensation
			...
			if (includeX || includeY || includeZ || includeRotation) {
				...
				/*
				 * If moveToCompleteRegex is specified we need to wait until we
				 * match the regex in a response before continuing. We first
				 * search the initial responses from the command for the regex.
				 * If it's not found we then collect responses for up to
				 * timeoutMillis while searching the responses for the regex. As
				 * soon as it is matched we continue. If it's not matched within
				 * the timeout we throw an Exception.
				 */
		...
		// regardless of any action above the subdriver needs its actions based
		// on original input
		for (ReferenceDriver driver : subDrivers) {
	...
	public String actuatorRead(ReferenceActuator actuator) throws Exception {
		...
		if (command == null || regex == null) {
			// If the command or regex is null we'll query the subdrivers. The
			// first
			...
			// If none of the subdrivers returned a value there's nothing left
			// to
			// do, so return null.
			return null;
		}
	...
	public List<String> sendCommand(String command, long timeout) throws Exception {
		...
		// Collect responses till we find one with the confirmation or we
		// timeout. Return
		...
		while (System.currentTimeMillis() - t < timeout) {
			// Wait to see if a response came in. We wait up until the number of
			// millis remaining
			// in the timeout.
		...
		// If a command was specified and no confirmation was found it's a
		// timeout error.
	...
	/**
	 * Find matches of variables in the format {Name:Format} and replace them
	 * with the specified value formatted using String.format with the specified
	 * Format. Format is optional and defaults to %s. A null value replaces the
	 * variable with "".
	 */
	...
	public PropertySheet[] getPropertySheets() {
		return new PropertySheet[] { new PropertySheetWizardAdapter(new GcodeDriverGcodes(this), "Gcode"),
	...
	public interface AxisTransform {
		/**
		 * Transform the specified raw coordinate into it's corresponding
		 * transformed coordinate. The transformed coordinate is what the user
		 * sees, while the raw coordinate is what the motion controller sees.
		 * 
		 * @param hm
		 * @param rawCoordinate
		 * @return
		...
		/**
		 * Transform the specified transformed coordinate into it's
		 * corresponding raw coordinate. The transformed coordinate is what the
		 * user sees, while the raw coordinate is what the motion controller
		 * sees.
		 * 
		 * @param hm
	 ...
	 * The two Z axes are defined as normal and negated. Normal gets the raw
	 * coordinate value and negated gets the same value negated. So, as normal
	 * moves up, negated moves down.
	 */
