...
public class OpenCvVisionProvider implements VisionProvider {
	...
	public Point[] locateTemplateMatches(int roiX, int roiY, int roiWidth, int roiHeight, int coiX, int coiY,
		...
		Logger.debug(String.format("locateTemplateMatches certainty %f at %f, %f", matchValue, matchLoc.x, matchLoc.y));
		...
		if (LogUtils.isDebugEnabled()) {
			try {
				Core.rectangle(roiImage, matchLoc,
						new org.opencv.core.Point(matchLoc.x + templateImage.cols(), matchLoc.y + templateImage.rows()),
						new Scalar(0, 255, 0));
