...
public class ReferenceBottomVisionPartConfigurationWizard extends AbstractConfigurationWizard {
	...
	private void testAlignment() throws Exception {
		...
		// Rotate the point 0,0 using the bottom offsets as a center point by
		// the angle
		// that is
		// the difference between the bottom vision angle and the calculated
		// global
		// placement angle.
		...
		// Set the angle to the difference mentioned above, aligning the part to
		// the
		// same angle as
		// the placement.
		location = location.derive(null, null, null, cameraLocation.getRotation() - offsets.getRotation());

		// Add the placement final location to move our local coordinate into
		// global
		// space
		location = location.add(cameraLocation);

		// Subtract the bottom vision offsets to move the part to the final
		// location,
		// instead of
		// the nozzle.
