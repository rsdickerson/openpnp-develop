...
public class SimulatedUpCamera extends ReferenceCamera implements Runnable {
	...
	public BufferedImage internalCapture() {
		...
		// determine if there are any nozzles within our bounds and if so render
		// them
	...
	private void drawNozzle(Graphics2D g, Nozzle nozzle) {
		...
		// Location l =
		// nozzle.getLocation().convertToUnits(LengthUnit.Millimeters);
		//
		// Location upp =
		// getUnitsPerPixel().convertToUnits(LengthUnit.Millimeters);
		// Location scale =
		// new Location(LengthUnit.Millimeters, 1D / upp.getX(), 1D /
		// upp.getY(), 0, 0);
		// l = l.multiply(scale);
		//
		...
		public void machineHeadActivity(Machine machine, Head head) {
			for (Nozzle nozzle : head.getNozzles()) {
				...
				if (part == null || part != nozzle.getPart()) {
					...
					Random r = new Random();
					offsets = new Location(LengthUnit.Millimeters, Math.random() * 2 - 1, Math.random() * 2 - 1, 0,
							Math.random() * 30 - 15);
				}
			}
		}
