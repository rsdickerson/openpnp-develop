...
public class ImageCamera extends ReferenceCamera implements Runnable {
	...
	public synchronized BufferedImage internalCapture() {
		...
		gFrame.drawImage(source, 0, 0, width - 1, height - 1, dx1, dy1, dx1 + width - 1, dy1 + height - 1, null);
	...
	private synchronized void initialize() throws Exception {
		...
		if (sourceUri.startsWith("classpath://")) {
			source = ImageIO.read(
					getClass().getClassLoader().getResourceAsStream(sourceUri.substring("classpath://".length())));
		} else {
