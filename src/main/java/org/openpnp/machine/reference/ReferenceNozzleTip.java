...
public class ReferenceNozzleTip extends AbstractNozzleTip {
	...
	public ReferenceNozzleTip() {
		Configuration.get().addListener(new ConfigurationListener.Adapter() {
			...
			public void configurationLoaded(Configuration configuration) throws Exception {
				...
				/*
				 * Backwards compatibility. Since this field is being added
				 * after the fact, if the field is not specified in the config
				 * then we just make a copy of the other mid location. The
				 * result is that if a user already has a changer configured
				 * they will not suddenly have a move to 0,0,0,0 which would
				 * break everything.
				 */
	...
	public boolean canHandle(Part part) {
		...
		// Logger.debug("{}.canHandle({}) => {}", getName(), part.getId(),
		// result);
		return result;
	}
	...
	public static class Calibration {
		...
		public void calibrate(ReferenceNozzleTip nozzleTip) throws Exception {
			...
			try {
				...
				for (int i = 0; i < 3; i++) {
					...
					// Subtract the offsets and move to that position to center
					// the nozzle.
					location = location.subtract(offset);
					nozzle.moveTo(location);
				}
				// This is our baseline location and should have the nozzle well
				// centered over the
				// camera.
				Location startLocation = location;

				// Now we rotate the nozzle 360 degrees at
				// calibration.angleIncrement steps, find the
				...
				// The nozzle tip is now calibrated and
				// calibration.getCalibratedOffset() can be
				// used.
		...
		private Location findCircle() throws Exception {
			...
			MainFrame.get().get().getCameraViews().getCameraView(camera)
					.showFilteredImage(OpenCvUtils.toBufferedImage(pipeline.getWorkingImage()), 250);
			return location;
		}

		/**
		 * Find the two closest offsets to the angle being requested. The
		 * offsets start at angle 0 and go to angle 360 - angleIncrement in
		 * angleIncrement steps.
		 */
		...
		public static CvPipeline createDefaultPipeline() {
			try {
				String xml = IOUtils.toString(
						ReferenceNozzleTip.class.getResource("ReferenceNozzleTip-Calibration-DefaultPipeline.xml"));
