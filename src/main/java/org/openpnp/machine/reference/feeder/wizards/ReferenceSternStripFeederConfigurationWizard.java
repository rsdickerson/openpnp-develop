...
import org.openpnp.machine.reference.feeder.ReferenceSternStripFeeder;
import org.openpnp.machine.reference.feeder.ReferenceSternStripFeeder.TapeType;
...
public class ReferenceSternStripFeederConfigurationWizard extends AbstractConfigurationWizard {
	private final ReferenceSternStripFeeder feeder;
	...
	public ReferenceSternStripFeederConfigurationWizard(ReferenceSternStripFeeder feeder) {
		this.feeder = feeder;
		...
		panelTapeSettings.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null),
		...
		panelLocations
				.setBorder(new TitledBorder(null, "Locations", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		locationButtonsPanelFeedStart = new LocationButtonsPanel(textFieldFeedStartX, textFieldFeedStartY,
				textFieldFeedStartZ, null);
		...
		public void actionPerformed(final CameraViewActionEvent action) {
			...
			Configuration.get().getMachine().submit(new Callable<Void>() {
				public Void call() throws Exception {
					...
					List<Location> referenceHoles = deriveReferenceHoles(part1HoleLocations, part2HoleLocations);
					...
					SwingUtilities.invokeLater(new Runnable() {
						public void run() {
							Helpers.copyLocationIntoTextFields(referenceHole1, textFieldFeedStartX, textFieldFeedStartY,
									null);
							Helpers.copyLocationIntoTextFields(referenceHole2, textFieldFeedEndX, textFieldFeedEndY,
									null);
	...
	private List<Location> findHoles(Camera camera) {
		...
		new FluentCv().setCamera(camera).settleAndCapture().toGray().blurGaussian(feeder.getHoleBlurKernelSize())
				.findCirclesHough(feeder.getHoleDiameterMin(), feeder.getHoleDiameterMax(), feeder.getHolePitchMin())
				.filterCirclesByDistance(feeder.getHoleDistanceMin(), feeder.getHoleDistanceMax())
				.filterCirclesToLine(feeder.getHoleLineDistanceMax()).convertCirclesToLocations(holeLocations);
	...
	private BufferedImage showHoles(Camera camera, BufferedImage image, boolean showDetails) {
		if (showDetails) {
			return new FluentCv().setCamera(camera).toMat(image, "original").toGray()
					...
					.drawCircles("unfiltered", Color.blue, "distanceFiltered").recall("houghDistanceFiltered")
					.filterCirclesToLine(feeder.getHoleLineDistanceMax()).drawCircles("distanceFiltered", Color.green)
					.toBufferedImage();
		} else {
			return new FluentCv().setCamera(camera).toMat(image, "original").toGray()
					.blurGaussian(feeder.getHoleBlurKernelSize())
					...
					.filterCirclesByDistance(feeder.getHoleDistanceMin(), feeder.getHoleDistanceMax())
					.filterCirclesToLine(feeder.getHoleLineDistanceMax()).drawCircles("original", Color.green)
					.toBufferedImage();
		}
	}
	...
	private List<Location> deriveReferenceHoles(List<Location> part1HoleLocations, List<Location> part2HoleLocations) {
		...
		// holes.
		Location part1ReferenceHole = VisionUtils.sortLocationsByDistance(part2HoleLocations.get(0), part1HoleLocations)
				.get(0);
		// Part 2's reference hole is the one farthest from part 1's reference
		// hole.
		Location part2ReferenceHole = Lists
				.reverse(VisionUtils.sortLocationsByDistance(part1ReferenceHole, part2HoleLocations)).get(0);
