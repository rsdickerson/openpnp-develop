...
public class ReferenceSlotAutoFeederConfigurationWizard extends AbstractConfigurationWizard {
	...
	public ReferenceSlotAutoFeederConfigurationWizard(ReferenceSlotAutoFeeder feeder) {
		...
		feederPanel.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Feeder",
		...
		panelActuator.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED, null, null), "Actuators",
				TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
	...
	public void createBindings() {
		...
		/**
		 * Note that we set up the bindings here differently than everywhere
		 * else. In most wizards the fields are bound with wrapped bindings and
		 * the proxy is bound with a hard binding. Here we do the opposite so
		 * that when the user captures a new location it is set on the proxy
		 * immediately. This allows the offsets to update immediately. I'm not
		 * actually sure why we do it the other way everywhere else, since this
		 * seems to work fine. Might not matter in most other cases.
		 */
		 ...
		 * We create an observable wrapper for bank and feeder. We add wrapped
		 * bindings for these against the source feeder, so if they are changed,
		 * then upon hitting Apply they will be changed on the source. In
		 * addition we add non-wrapped bindings from the bank and feeder
		 * wrappers to their instance properties such as name and part. Thus
