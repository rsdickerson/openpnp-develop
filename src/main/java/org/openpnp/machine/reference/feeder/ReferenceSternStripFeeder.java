...
import org.openpnp.machine.reference.feeder.wizards.ReferenceSternStripFeederConfigurationWizard;
 ...
 */
public class ReferenceSternStripFeeder extends ReferenceFeeder {
	...
	public Location getPickLocation() throws Exception {
		...
		/*
		 * As a special case, before the feeder has been fed we return the pick
		 * location as if the feeder had been fed. This keeps us from returning
		 * a pick location that is off the edge of the strip.
		 */
		if (feedCount == 0) {
			feedCount = 1;
		}
		...
		// partPitchAdjusted is the euclidian distance of ReferenceHole and
		// NextHole and divided by
		// the amount of part locations in between. This Part count is derived
		// from the distance
		// and the given partPitch in GUI and afterwards rounded to the next
		// integer value.
		// partPitch/partPitchAdjusted limitation
		// It's the P1 value according to EIA-481-C, October 2003, pg. 9, 11, 13
		// Accuracy variations as specified in the document are not taken into
		// account!
	...
	private Location findClosestHole(Camera camera) {
		...
		BufferedImage image = new FluentCv().setCamera(camera).settleAndCapture("original")
				.saveDebugImage(ReferenceSternStripFeeder.class, "findClosestHole", "original").toGray()
				.blurGaussian(getHoleBlurKernelSize())
				.findCirclesHough(getHoleDiameterMin(), getHoleDiameterMax(), getHolePitchMin(), "circles")
				.convertCirclesToLocations(holeLocations).drawCircles("original")
				.saveDebugImage(ReferenceSternStripFeeder.class, "findClosestHole", "debug").toBufferedImage();
	...
	public Wizard getConfigurationWizard() {
		return new ReferenceSternStripFeederConfigurationWizard(this);
	}
