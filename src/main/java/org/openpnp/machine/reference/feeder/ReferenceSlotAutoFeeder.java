...
public class ReferenceSlotAutoFeeder extends ReferenceAutoFeeder {
	...
	public ReferenceSlotAutoFeeder() {
		...
		this.name = this.id;
		// partId is required in AbstractFeeder to save the config. We don't use
		// it so we just
	...
	public Location getPickLocation() throws Exception {
		...
		// Add the rotated offsets to the pickLocation to get the final
		// pickLocation. We add
		// with rotation since we need to pick with the combined rotation of the
		// slot and the
		// offsets.
	...
	public static synchronized IdentifiableList<Bank> getBanks() {
		BanksProperty bp = (BanksProperty) Configuration.get().getMachine()
				.getProperty("ReferenceAutoFeederSlot.banks");
		if (bp == null) {
	...
	public void setName(String name) {
		 ...
		 * https://github.com/openpnp/openpnp/issues/427. It removes any
		 * instance of the additional info we show in the name when a feeder is
		 * attached.
		 */
