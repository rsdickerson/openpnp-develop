 ...
 * Location, performing a template match against the Template Image bound by the
 * Area of Interest and then storing the offsets from the Pick Location to the
 * matched image as Vision Offsets.
 * 
 * The feed operation consists of: 1. Apply the Vision Offsets to the Feed Start
 * Location and Feed End Location. 2. Feed the tape with the modified Locations.
 * 3. Perform the Vision Operation. 4. Apply the new Vision Offsets to the Pick
 * Location and return the Pick Location for Picking.
 * 
 * This leaves the head directly above the Pick Location, which means that when
 * the Feeder is then commanded to pick the Part it only needs to move the
 * distance of the Vision Offsets and do the pick. The Vision Offsets are then
 * used in the next feed operation to be sure to hit the tape at the right
 * position.
 */
public class ReferenceDragFeeder extends ReferenceFeeder {
	...
	private Location getVisionOffsets(Head head, Location pickLocation) throws Exception {
		...
		Point[] matchingPoints = visionProvider.locateTemplateMatches(aoi.getX(), aoi.getY(), aoi.getWidth(),
				aoi.getHeight(), 0, 0, vision.getTemplateImage());
