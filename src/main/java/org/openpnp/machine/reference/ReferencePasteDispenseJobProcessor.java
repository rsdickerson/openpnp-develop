...
public class ReferencePasteDispenseJobProcessor extends AbstractPasteDispenseJobProcessor {
	enum State {
		Uninitialized, PreFlight, FiducialCheck, Dispense, Cleanup, Stopped
	}
	...
	public static class JobDispense {
		...
		public double getDistance() {
			Location zeroLocation = this.boardLocation.getLocation().derive((double) 0, (double) 0, (double) 0,
					(double) 0);

			return zeroLocation.getLinearDistanceTo(
					(Utils2D.calculateBoardPlacementLocation(this.boardLocation, this.boardPad.getLocation())));
		}
	}
	...
	public synchronized boolean next() throws Exception {
		...
		if (fsm.getState() == State.Stopped) {
			...
			/*
			 * If we've reached the Dispense state and there are no more
			 * dispenses to work on the job is complete. We send the Complete
			 * Message to start the cleanup process.
			 */
	...
	protected void doPreFlight() throws Exception {
		...
		for (BoardLocation boardLocation : job.getBoardLocations()) {
			...
			for (BoardPad pad : boardLocation.getBoard().getSolderPastePads()) {
				...
				// Ignore pads that aren't on the side of the board we're
				// processing.
		...
		// Do a very basic sort by distance to stop the machine going randomly
		// round the PCB
	...
	protected void doDispense() throws Exception {
		for (JobDispense jobDispense : jobDispenses) {
			/*
			 * if (jobDispense.stepComplete) { continue; }
			 */
			...
			Location dispenseLocation = Utils2D.calculateBoardPlacementLocation(boardLocation, boardPad.getLocation());
	...
	/*
	 * @Override public Wizard getConfigurationWizard() { return new Ref(this);
	 * }
	 * 
	 * public boolean isParkWhenComplete() { return parkWhenComplete; }
	 * 
