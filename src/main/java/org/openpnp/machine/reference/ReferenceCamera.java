...
public abstract class ReferenceCamera extends AbstractCamera implements ReferenceHeadMountable {
	...
	/**
	 * Wraps internalCapture() to ensure that a null image is never returned.
	 * Attempts to retry capture if the capture returns null and if no image can
	 * be captured returns a default image. Several of the low level camera
	 * drivers return null when there is a capture error, but these are often
	 * temporary and we would prefer not to have bad images returned. The retry
	 * is intended to smooth this out.
	 * 
	 * @return
	 */
	...
	private Mat crop(Mat mat) {
		if (cropWidth != 0 || cropHeight != 0) {
			...
			Rect roi = new Rect((int) ((mat.size().width / 2) - (cw / 2)), (int) ((mat.size().height / 2) - (ch / 2)),
					cw, ch);
	...
	private Mat calibrate(Mat mat) {
		...
		// If the count changed then we have counted a new image, so let the
		// caller know.
