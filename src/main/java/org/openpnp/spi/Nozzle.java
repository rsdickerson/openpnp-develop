...
public interface Nozzle extends HeadMountable, WizardConfigurable, PropertySheetHolder {
	 ...
	 * If this returns without Exception then subsequently calling
	 * getNozzleTip() should return the same NozzleTip as was passed to this
	 * call.
	 * 
	 * If the specified NozzleTip is already loaded this method should do
	 * nothing.
	 * 
	 * @param nozzleTip
	 * @throws Exception
	...
	/**
	 * Get the part that is currently picked on the Nozzle, or null if none is
	 * picked. Should typically be non-null after a pick operation and before a
	 * place operation and null after a pick operation. Of note, it should be
	 * non-null after a failed pick operation so that the system can determine
	 * which part it may need to discard. It may also be null if a user
