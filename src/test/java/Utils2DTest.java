...
public class Utils2DTest {
	...
	public void testCalculateBoardLocationRandom() throws Exception {
		for (int i = 0; i < 100000; i++) {
			// The actual board location that we are looking for. This is where
			// the board actually
			// is on the table.
			...
			// Given the actual board location and the two placements, calculate
			// where they should
			...
			// Using a random input board location, which should not actually
			// affect the result,
			// calculate the board location from the known actual locations. The
			// result
			...
			Location location = Utils2D.calculateBoardLocation(testBoardLocation, placementA, placementB,
			...
			try {
				...
				throw new Exception(String.format(
						...
						actualBoardLocation, placementA, placementB, testBoardLocation, location), e);
			}
		}
	}
	...
	private static void check(Location results, double x, double y, double z, double c) throws Exception {
	...
	private static void checkNormalized(Location results, double x, double y, double z, double c) throws Exception {
		within("angle", Utils2D.normalizeAngle(results.getRotation()), Utils2D.normalizeAngle(c), 0.001);
		within("x", results.getX(), x, 0.01);
		within("y", results.getY(), y, 0.01);
		within("z", results.getZ(), z, 0.01);
	}
